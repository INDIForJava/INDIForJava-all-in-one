...
public class TestVector extends TestGsl {
    ...
    void test_func(int stride, int N) {
...

        {
            ...
            for (i = 0; i < N; i++) {
                if (v.ptr(i).equals(new DoubleArray(v.data, i * stride)))
                    status = false;
            }
...

        {
            ...
            status = (v.isNull());
            gsl_test(status, "_isnull" + "gsl_vector" + " on non-negative vector");

            status = (v.isPos());
            gsl_test(status, "_ispos" + "gsl_vector" + " on non-negative vector");

            status = (v.isNeg());
            gsl_test(status, "_isneg" + "gsl_vector" + " on non-negative vector");

            status = (!v.isNonNeg());
            gsl_test(status, "_isnonneg" + "gsl_vector" + " on non-negative vector");
        }
...

        {
            ...
            status = (v.isNull());
            gsl_test(status, "_isnull" + "gsl_vector" + " on non-positive vector");

            status = (v.isPos());
            gsl_test(status, "_ispos" + "gsl_vector" + " on non-positive vector");

            status = (v.isNeg());
            gsl_test(status, "_isneg" + "gsl_vector" + " on non-positive non-null vector");

            status = (v.isNonNeg());
            gsl_test(status, "_isnonneg" + "gsl_vector" + " on non-positive non-null vector");
        }
...

        {
            ...
            for (i = 0; i < N / 2; i++) {
                if (v1.vector.get(i) != v.get((N / 3) + i))
                    status = true;
            }
