...
public class CCDDriverExtention extends INDIDriverExtention<INDICCD> {
	...
	private static final Logger LOG = Logger.getLogger(CCDDriverExtention.class
			.getName());
	static double P0 = 0.906, P1 = 0.584, P2 = 0.365, P3 = 0.117, P4 = 0.049,
			P5 = -0.05, P6 = -0.064, P7 = -0.074, P8 = -0.094;

	static private int[][] PROXIMITY_MATRIX = { { 8, 8, 8, 8, 8, 8, 8, 8, 8 },
			{ 8, 8, 8, 7, 6, 7, 8, 8, 8 }, { 8, 8, 5, 4, 3, 4, 5, 8, 8 },
			{ 8, 7, 4, 2, 1, 2, 4, 8, 8 }, { 8, 6, 3, 1, 0, 1, 3, 6, 8 },
			{ 8, 7, 4, 2, 1, 2, 4, 8, 8 }, { 8, 8, 5, 4, 3, 4, 5, 8, 8 },
			{ 8, 8, 8, 7, 6, 7, 8, 8, 8 }, { 8, 8, 8, 8, 8, 8, 8, 8, 8 } };
	private final SimpleDateFormat dateFormatISO8601 = new SimpleDateFormat(
	...
	public CCDDriverExtention(INDICCD indiccd) {
		...
		imageExposure.setEventHandler(new NumberEvent() {
			...
			@Override
			public void processNewValue(Date date,
	...
	private void newImageExposureValue(
			INDINumberElementAndValue[] elementsAndValues) {
	...
	private void newImageFrameValue(
			INDINumberElementAndValue[] elementsAndValues) {
		...
		LOG.log(Level.FINE, String.format(
				"Requested CCD Frame is %4.0f,%4.0f %4.0f x %4.0f",
				imageFrameX.getIntValue(), imageFrameY.getIntValue(),
				imageFrameWidth.getIntValue(), imageFrameHeigth.getIntValue()));

		if (!driverInterface.updateCCDFrame(imageFrameX.getIntValue(),
				imageFrameY.getIntValue(), imageFrameWidth.getIntValue(),
	...
	private void newRapidGuideDataValue(
			INDINumberElementAndValue[] elementsAndValues) {
	...
	private void newCompressedValue(
			INDISwitchElementAndValue[] elementsAndValues) {
	...
	private void newRapidGuideValue(
			INDISwitchElementAndValue[] elementsAndValues) {
	...
	private void newRapidGuideSetupValue(
			INDISwitchElementAndValue[] elementsAndValues) {
	...
	protected boolean exposureComplete() {
		boolean sendImage = (driver.uploadClient.getValue() == SwitchStatus.ON || driver.uploadBoth
				.getValue() == SwitchStatus.ON);
		boolean saveImage = (driver.uploadLocal.getValue() == SwitchStatus.ON || driver.uploadBoth
				.getValue() == SwitchStatus.ON);
		...
		if (sendData) {
			...
			int[][] src = ccdImage.subImageInt(minx - 4, miny - 4, maxx + 4,
					maxy + 4);
				...
				for (int x = 4; x < (src[x].length - 4); x++) {
					...
					for (int My = 0; My < PROXIMITY_MATRIX.length; My++) {
						for (int Mx = 0; Mx < PROXIMITY_MATRIX[My].length; Mx++) {
							i[PROXIMITY_MATRIX[My][Mx]] = src[y + My - 4][x
									+ Mx - 4];
						}
					}
					average = (i[0] + i[1] + i[2] + i[3] + i[4] + i[5] + i[6]
							+ i[7] + i[8]) / 85.0;
					fit = P0 * (i[0] - average) + //
							P1 * (i[1] - 4 * average) + //
							P2 * (i[2] - 4 * average) + //
			...
			if (bestFit > 50) {
				if (total > 0) {
					...
					LOG.log(Level.FINE, String.format(
							"Guide Star X: %g Y: %g FIT: %g",
							rapidGuideDataX.getValue(),
							rapidGuideDataY.getValue(),
							rapidGuideDataFIT.getValue()));
				} else {
	 ...
	 */
	protected void addFITSKeywords(CCDDriverExtention targetChip,
			BasicHDU fitsHeader) throws HeaderCardException {
		fitsHeader.addValue("EXPTIME", targetChip.exposureDuration,
				"Total Exposure Time (s)");

		if (targetChip.getFrameType() == CCD_FRAME.DARK_FRAME)
			fitsHeader.addValue("DARKTIME", targetChip.exposureDuration,
					"Total Exposure Time (s)");

		fitsHeader.addValue("PIXSIZE1", targetChip.getPixelSizeX(),
				"Pixel Size 1 (microns)");
		fitsHeader.addValue("PIXSIZE2", targetChip.getPixelSizeY(),
				"Pixel Size 2 (microns)");
		fitsHeader.addValue("XBINNING", targetChip.BinX,
				"Binning factor in width");
		fitsHeader.addValue("YBINNING", targetChip.BinY,
				"Binning factor in height");
		fitsHeader.addValue("FRAME", targetChip.getFrameType().fitsValue(),
				"Frame Type");
		...
		fitsHeader.addValue("DATE-OBS", targetChip.getExposureStartTime(),
