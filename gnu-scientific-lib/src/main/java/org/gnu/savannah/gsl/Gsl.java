...
public class Gsl {
    ...
    public static double cblas_ddot(int N, DoubleArray X, int incX, DoubleArray Y, int incY) {
    ...
    public static void cblas_dgemm(CBLAS_ORDER Order, CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB, int M, int N, int K, double alpha, DoubleArray A, int lda,
        ...
        if (Order == CBLAS_ORDER.CblasRowMajor) {
            ...
            F = A;
            ldf = lda;
            TransF = (TransA == CBLAS_TRANSPOSE.CblasConjTrans) ? CBLAS_TRANSPOSE.CblasTrans : TransA;
            G = B;
            ldg = ldb;
            TransG = (TransB == CBLAS_TRANSPOSE.CblasConjTrans) ? CBLAS_TRANSPOSE.CblasTrans : TransB;
        } else {
            n1 = N;
            n2 = M;
            F = B;
            ldf = ldb;
            TransF = (TransB == CBLAS_TRANSPOSE.CblasConjTrans) ? CBLAS_TRANSPOSE.CblasTrans : TransB;
            G = A;
            ldg = lda;
            TransG = (TransA == CBLAS_TRANSPOSE.CblasConjTrans) ? CBLAS_TRANSPOSE.CblasTrans : TransA;
        }
        ...
        if (TransF == CBLAS_TRANSPOSE.CblasNoTrans && TransG == CBLAS_TRANSPOSE.CblasNoTrans) {
            ...
            for (k = 0; k < K; k++) {
                for (i = 0; i < n1; i++) {
                    ...
                    if (temp != 0.0) {
                        for (j = 0; j < n2; j++) {
                            C.set(ldc * i + j, C.get(ldc * i + j) + temp * G.get(ldg * k + j));
                        }
                    }
                }
            }

        } else if (TransF == CBLAS_TRANSPOSE.CblasNoTrans && TransG == CBLAS_TRANSPOSE.CblasTrans) {
        ...
        } else if (TransF == CBLAS_TRANSPOSE.CblasTrans && TransG == CBLAS_TRANSPOSE.CblasNoTrans) {
        ...
        } else if (TransF == CBLAS_TRANSPOSE.CblasTrans && TransG == CBLAS_TRANSPOSE.CblasTrans) {
    ...
    public static void cblas_dgemv(CBLAS_ORDER order, CBLAS_TRANSPOSE TransA, int M, int N, double alpha, DoubleArray A, int lda, DoubleArray X, int incX, double beta,
        ...
        // #define double double
        {
            ...
            if ((order == CBLAS_ORDER.CblasRowMajor && Trans == CBLAS_TRANSPOSE.CblasNoTrans) || (order == CBLAS_ORDER.CblasColMajor && Trans == CBLAS_TRANSPOSE.CblasTrans)) {
                /* form y := alpha*A*x + y */
                int iy = offset(lenY, incY);
                for (i = 0; i < lenY; i++) {
                    double temp = 0.0;
                ...
                throw new UnsupportedOperationException("unrecognized operation");
            }
        }
        // #undef double
    }
    ...
    public static void cblas_drot(int N, DoubleArray X, int incX, DoubleArray Y, int incY, double c, double s) {
    ...
    public static void cblas_dtrsv(CBLAS_ORDER order, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, int N, DoubleArray A, int lda, DoubleArray X, int incX) {

        {
            ...
            if ((order == CBLAS_ORDER.CblasRowMajor && Trans == CBLAS_TRANSPOSE.CblasNoTrans && Uplo == CBLAS_UPLO.CblasUpper)
                    || (order == CBLAS_ORDER.CblasColMajor && Trans == CBLAS_TRANSPOSE.CblasTrans && Uplo == CBLAS_UPLO.CblasLower)) {
                /* backsubstitution */
            ...
            } else if ((order == CBLAS_ORDER.CblasRowMajor && Trans == CBLAS_TRANSPOSE.CblasNoTrans && Uplo == CBLAS_UPLO.CblasLower)
                    || (order == CBLAS_ORDER.CblasColMajor && Trans == CBLAS_TRANSPOSE.CblasTrans && Uplo == CBLAS_UPLO.CblasUpper)) {
            ...
            } else if ((order == CBLAS_ORDER.CblasRowMajor && Trans == CBLAS_TRANSPOSE.CblasTrans && Uplo == CBLAS_UPLO.CblasUpper)
                    || (order == CBLAS_ORDER.CblasColMajor && Trans == CBLAS_TRANSPOSE.CblasNoTrans && Uplo == CBLAS_UPLO.CblasLower)) {
            ...
            } else if ((order == CBLAS_ORDER.CblasRowMajor && Trans == CBLAS_TRANSPOSE.CblasTrans && Uplo == CBLAS_UPLO.CblasLower)
                    || (order == CBLAS_ORDER.CblasColMajor && Trans == CBLAS_TRANSPOSE.CblasNoTrans && Uplo == CBLAS_UPLO.CblasUpper)) {
    ...
    private static void chase_out_intermediate_zero(GslVector d, GslVector f, GslMatrix U, int k0) {
    ...
    /*
     * Factorise a general M x N matrix A into, A = U D V^T where U is a
     * column-orthogonal M x N matrix (U^T U = I), D is a diagonal N x N matrix,
     * and V is an N x N orthogonal matrix (V^T V = V V^T = I) U is stored in
     * the original matrix A, which has the same size V is stored as a separate
     * matrix (not V^T). You must take the transpose to form the product above.
     * The diagonal matrix D is stored in the vector S, D_ii = S_i
     */

    private static void chase_out_trailing_zero(GslVector d, GslVector f, GslMatrix V) {
    ...
    private static void chop_small_elements(GslVector d, GslVector f) {
        ...
        for (i = 0; i < N - 1; i++) {
            ...
            if (Math.abs(f_i) < GSL_DBL_EPSILON * (Math.abs(d_i) + Math.abs(d_ip1))) {
                f.set(i, 0.0);
            }
    ...
    private static void create_schur(double d0, double f0, double d1, DoubleRef c, DoubleRef s) {
        ...
        if (Math.abs(d0) < GSL_SQRT_DBL_MIN || Math.abs(d0) > GSL_SQRT_DBL_MAX || Math.abs(f0) < GSL_SQRT_DBL_MIN || Math.abs(f0) > GSL_SQRT_DBL_MAX
                || Math.abs(d1) < GSL_SQRT_DBL_MIN || Math.abs(d1) > GSL_SQRT_DBL_MAX) {
            double scale;
    ...
    public static GslErrno gsl_blas_daxpy(double alpha, GslVector X, GslVector Y) {
        if (X.size == Y.size) {
    ...
    public static GslErrno gsl_blas_ddot(GslVector X, GslVector Y, DoubleRef result) {
        if (X.size == Y.size) {
            result.value = cblas_ddot(X.size, X.data, X.stride, Y.data, Y.stride);
            return GslErrno.GSL_SUCCESS;
        } else {
            throw new IllegalStateException("invalid length");
        }
    ...
    public static GslErrno gsl_blas_dgemm(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB, double alpha, GslMatrix A, GslMatrix B, double beta, GslMatrix C) {
        int M = C.size1;
        int N = C.size2;
    ...
    public static GslErrno gsl_blas_dgemv(CBLAS_TRANSPOSE TransA, double alpha, GslMatrix A, GslVector X, double beta, GslVector Y) {
        int M = A.size1;
        int N = A.size2;

        if ((TransA == CBLAS_TRANSPOSE.CblasNoTrans && N == X.size && M == Y.size) || (TransA == CBLAS_TRANSPOSE.CblasTrans && M == X.size && N == Y.size)) {
            cblas_dgemv(CBLAS_ORDER.CblasRowMajor, TransA, M, N, alpha, A.data, A.tda, X.data, X.stride, beta, Y.data, Y.stride);
            return GslErrno.GSL_SUCCESS;
        } else {
            throw new IllegalStateException("invalid length");
        }
    ...
    public static GslErrno gsl_blas_drot(GslVector X, GslVector Y, double c, double s) {
        if (X.size == Y.size) {
    ...
    public static GslErrno gsl_blas_dtrsv(CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag, GslMatrix A, GslVector X) {
        int M = A.size1;
        int N = A.size2;
        ...
        cblas_dtrsv(CBLAS_ORDER.CblasRowMajor, Uplo, TransA, Diag, N, A.data, A.tda, X.data, X.stride);
        return GslErrno.GSL_SUCCESS;
    }
    ...
    public static GslErrno gsl_linalg_bidiag_decomp(GslMatrix A, GslVector tau_U, GslVector tau_V) {
        if (A.size1 < A.size2) {
            throw new IllegalStateException("bidiagonal decomposition requires M>=N");
            ...
            for (i = 0; i < N; i++) {
...

                {
                    ...
                    if (i + 1 < N) {
                        GslMatrixView m = A.submatrix(i, i + 1, M - i, N - (i + 1));
                ...
                if (i + 1 < N) {
                    ...
                    GslVectorView v = r.vector.subvector(i + 1, N - (i + 1));
                    double tau_i = gsl_linalg_householder_transform(v.vector);

                    /* Apply the transformation to the remaining rows */

                    if (i + 1 < M) {
                        GslMatrixView m = A.submatrix(i + 1, i + 1, M - (i + 1), N - (i + 1));
    ...
    public static GslErrno gsl_linalg_bidiag_unpack2(GslMatrix A, GslVector tau_U, GslVector tau_V, GslMatrix V) {
        int M = A.size1;
        int N = A.size2;
        ...
        if (M < N) {
            ...
            for (i = N - 1; i-- > 0;) {
                ...
                GslMatrixView m = V.submatrix(i + 1, i + 1, N - (i + 1), N - (i + 1));
     ...
     */
    private static void gsl_linalg_givens(double a, double b, DoubleRef c, DoubleRef s) {
        if (b == 0) {
            c.value = 1;
            s.value = 0;
    ...
    public static GslErrno gsl_linalg_householder_hm(double tau, GslVector v, GslMatrix A) {
...

        {
            ...
            GslMatrixView A1 = A.submatrix(1, 0, A.size1 - 1, A.size2);
            int j;
    ...
    public static GslErrno gsl_linalg_householder_mh(double tau, GslVector v, GslMatrix A) {
        /*
        ...
        {
            GslVectorView v1 = v.subvector(1, v.size - 1);
            GslMatrixView A1 = A.submatrix(0, 1, A.size1, A.size2 - 1);
            int i;
     ...
     * x(1:n-1) Inputs: v - on input, x vector on output, householder vector v
     * Notes: 1) on output, v is normalized so that v[0] = 1. The 1 is not
     * actually stored; instead v[0] = -sign(x[0])*||x|| so that: P x = v[0] *
     * e_1 Therefore external routines should take care when applying the
     * projection matrix P to vectors, taking into account that v[0] should be 1
     * when doing so.
     */
    ...
    /**
     * Factorise a general N x N matrix A into, P A = L U where P is a
     ...
     * diagonal elements of L are unity and are not stored. U is stored in the
     * diagonal and upper triangular part of the input matrix. P is stored in
    ...
    public static GslErrno gsl_linalg_LU_decomp(GslMatrix A, GslPermutation p, IntegerRef signum) {
        if (A.size1 != A.size2) {
            ...
            throw new IllegalStateException("permutation length must match matrix size");
        }
        int N = A.size1;
        int i, j, k;
        signum.value = 1;
    ...
    public static GslErrno gsl_linalg_LU_invert(GslMatrix LU, GslPermutation p, GslMatrix inverse) {
        int i, n = LU.size1;
    ...
    public static GslErrno gsl_linalg_LU_svx(GslMatrix LU, GslPermutation p, GslVector x) {
        if (LU.size1 != LU.size2) {
            ...
            throw new IllegalStateException("matrix size must match solution/rhs size");
            ...
            gsl_blas_dtrsv(CBLAS_UPLO.CblasLower, CBLAS_TRANSPOSE.CblasNoTrans, CBLAS_DIAG.CblasUnit, LU, x);

            /* Perform back-substitution, U x = c */

            gsl_blas_dtrsv(CBLAS_UPLO.CblasUpper, CBLAS_TRANSPOSE.CblasNoTrans, CBLAS_DIAG.CblasNonUnit, LU, x);

            return GslErrno.GSL_SUCCESS;
        }
    }

    public static GslErrno gsl_linalg_SV_decomp(GslMatrix A, GslMatrix V, GslVector S, GslVector work) {
        int a, b, i, j, iter;
        ...
        if (M < N) {
            throw new IllegalStateException("svd of MxN matrix, M<N, is not implemented");
        } else if (V.size1 != N) {
            throw new IllegalStateException("square matrix V must match second dimension of matrix A");
        } else if (V.size1 != V.size2) {
            throw new IllegalStateException("matrix V must be square");
        } else if (S.size != N) {
            throw new IllegalStateException("length of vector S must match second dimension of matrix A");
        } else if (work.size != N) {
            throw new IllegalStateException("length of workspace must match second dimension of matrix A");
        }
...

        {
            ...
            while (b > 0) {
                ...
                if (iter > 100 * N) {
                    throw new IllegalStateException("SVD decomposition failed to converge");
                }

                {
                    int n_block = b - a + 1;
                    GslVectorView S_block = S.subvector(a, n_block);
                    GslVectorView f_block = f.vector.subvector(a, n_block - 1);

                    GslMatrixView U_block = A.submatrix(0, a, A.size1, n_block);
                    GslMatrixView V_block = V.submatrix(0, a, V.size1, n_block);
    ...
    private static void qrstep(GslVector d, GslVector f, GslMatrix U, GslMatrix V) {
        int n = d.size;
        double y, z;
    ...
    private static void svd2(GslVector d, GslVector f, GslMatrix U, GslMatrix V) {
        int i;
        DoubleRef c = new DoubleRef(), s = new DoubleRef();
    ...
    private static double trailing_eigenvalue(GslVector d, GslVector f) {
...

        {
             ...
             * possibility of NaNs in the formula above. The matrix is [ da^2 +
             * fa^2, da fb ; da fb , db^2 + fb^2 ] and mu is the eigenvalue
             * closest to the bottom right element.
             */
